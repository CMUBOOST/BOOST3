...
int main(int argc, char **argv)
{
	...
	while (ros::ok())
	{
		...
		ROS_DEBUG_STREAM("Heading is " << heading / toRadians);
        	...
        	if (posQ->size == sample_size) avg_heading = averageHeading(posQ);

        	ROS_INFO_STREAM("Average Heading is " << avg_heading / toRadians);
		...
		if (posQ->size == sample_size)
		{
			...
			if (fabs(ros::Time::now().toSec() - currTime) < MAX_TIME)
				...
				for (int j = 1; j <= posQ->size; j++) 
				{
					printf("%d:  x: %05.10f  y: %05.10f", j, cur2->x, cur2->y);
